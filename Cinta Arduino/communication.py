    ...
    def _monitor_loop(self):
        ...
        while self.is_monitoring and self.is_connected:
            try:
                ...
                print(f"Error en bucle de monitoreo ({consecutive_errors}/{max_consecutive_errors}): {e}")
...
def example_usage(port: str = None):
    ...
    if port is None:
        ...
        if port not in available_ports:
            print(f"El puerto {port} no está disponible. Puertos disponibles: {available_ports}")
            return
